...
impl DmaRegs {
    ...
    pub fn on_write_control(&mut self, size: RvSize, val: RvData) -> Result<(), BusError> {
        ...
        if self.control.reg.is_set(Control::GO) {
            ...
            self.status0
                .reg
                .write(Status0::DMA_FSM_PRESENT_STATE::WAIT_DATA + Status0::BUSY::SET);
        }
    ...
    pub fn on_read_data(&mut self, size: RvSize) -> Result<RvData, BusError> {
        let popped: Vec<u8> = self.fifo.drain(..size as usize).collect();
        let data = match size {
            RvSize::Byte => u8::from_le_bytes(popped.as_slice().try_into().unwrap()) as u32,
            RvSize::HalfWord => u16::from_le_bytes(popped.as_slice().try_into().unwrap()) as u32,
            RvSize::Word => u32::from_le_bytes(popped.as_slice().try_into().unwrap()),
    ...
    pub fn do_dma_handling(&mut self, root_bus: &mut CaliptraRootBus) {
        ...
        self.status0
            .reg
            .write(Status0::DMA_FSM_PRESENT_STATE::DONE + Status0::BUSY::CLEAR);
    }
}
...
mod tests {
    ...
    use crate::CaliptraRootBusArgs;

    const DMA_BASE_ADDR: u32 = 0x3002_2000;
    const RRI_BASE_ADDR: u32 = 0x1003_8000;

    const NAME_OFFSET: u32 = 0;
    const NAME_VAL: u32 = 0x6776_8068;
    const CAPABILITIES_OFFSET: u32 = 4;
    const CONTROL_OFFSET: u32 = 8;
    const STATUS0_OFFSET: u32 = 0xc;
    const SOURCE_ADDR_LOW_OFFSET: u32 = 0x14;
    const BYTE_COUNT_OFFSET: u32 = 0x24;
    const READ_DATA_OFFSET: u32 = 0x30;
    ...
    fn test_dma_fifo() {
        ...
        let mut root_bus = CaliptraRootBus::new(&clock, args);

        const RRI_PROTCAP_OFFSET: u32 = 8;

        let mut dma = root_bus.dma.clone();
        dma.write(
            RvSize::Word,
            SOURCE_ADDR_LOW_OFFSET,
            RRI_BASE_ADDR + RRI_PROTCAP_OFFSET,
        )
        .unwrap();
        dma.write(RvSize::Word, BYTE_COUNT_OFFSET, 1).unwrap();
        dma.write(RvSize::Word, CONTROL_OFFSET, 0x20001).unwrap(); // Read to FIFO and go
        let busy = dma.read(RvSize::Word, STATUS0_OFFSET).unwrap();
        assert!(busy & 1 == 1);

        while dma.read(RvSize::Word, STATUS0_OFFSET).unwrap() & 1 == 1 {
            clock.increment_and_process_timer_actions(1, &mut root_bus);
        }

        let read_data = dma.read(RvSize::Word, READ_DATA_OFFSET).unwrap();
        assert!(read_data == 1); // 1.0 TODO prot_cap_magic would be better
    }
}
